...
std::string execPythonScript(){
    ...
    std::string result;
    std::unique_ptr<FILE,decltype(&pclose)> pipe(popen("python python_called.py","r"),pclose);
    if(!pipe){
...
// Compile with:
// g++ try.cpp -o try -I "/c/Program Files (x86)/SQLiteCpp/include" -L "/c/Program Files (x86)/SQLiteCpp/lib" -lSQLiteCpp -lsqlite3 
